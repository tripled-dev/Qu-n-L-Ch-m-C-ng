...
if (!code.includes('calculateKpiFromScores')) {
  code = code.replace(
...
const target = `      // Existing slip bonus
...
const replacement = `      // Existing slip bonus
      const existingSlip = payrollSlips.find(p => p.staffId === staff.id && p.month === currentMonth);
      const totalBonus = (existingSlip?.generalBonus ?? 0) + bonusQty;
...
if (code.includes(target)) {
  ...
  console.log("TimesheetManager updated successfully!");
} else {
  console.log("Error: target not found in TimesheetManager.tsx");
}
