    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">
              Số Ngày Công Chuẩn Trong Tháng (cho Trợ lý):
            </label>
            <input
              type="number"
              value={formData.defaultWorkingDaysInMonth}
              onChange={e => setFormData({ ...formData, defaultWorkingDaysInMonth: Number(e.target.value) })}
              className="w-32 border border-slate-300 rounded-lg px-3 py-2 text-sm font-bold text-center"
            />
          </div>
      ...
      {/* Google Sheets Sync & GitHub Deploy Shortcut Card */}
      <div className="bg-gradient-to-br from-emerald-900 via-teal-900 to-slate-900 text-white rounded-xl p-6 shadow-md space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-emerald-300">
              <FileSpreadsheet className="w-5 h-5" />
              <h4 className="font-bold text-base text-white">
                Đồng Bộ Đám Mây Google Sheet & Deploy GitHub
              </h4>
            </div>
            <p className="text-xs text-emerald-200/80 max-w-xl">
              Lưu trữ toàn bộ bảng chấm công, phiếu lương và danh sách nhân sự trực tiếp trên Google Sheet của bạn. Triển khai web lên Vercel/Netlify miễn phí.
            </p>
          </div>
          <button
            type="button"
            onClick={() => setActiveTab('gsheet')}
            className="px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs rounded-lg transition-all flex items-center justify-center gap-2 whitespace-nowrap"
          >
            <span>Mở Bảng Đồng Bộ Google Sheet</span>
