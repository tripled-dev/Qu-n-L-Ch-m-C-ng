...
export const StaffManager: React.FC = () => {
  ...
  const handleOpenModal = (staff?: Staff) => {
    ...
    if (staff) {
      ...
      setFormData({
        ...
        roles: ['giang_vien'],
        assignedChecklistIds: defaultRole.defaultChecklistIds || ['chk_day_hoc'],
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    const isExamCrafter = formData.roles.includes('soan_de_thi');
    ...
    else if (isExamCrafter) baseRate = rates.gradingRate || 150000;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredStaff.map(staff => {
          ...
          const hasExamCrafter = hasStaffRole(staff, 'soan_de_thi');
          ...
          return (
            ...
            >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {(staff.citizenId || staff.cccd) && (
                          <span className="text-[9px] font-mono text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200 hidden sm:inline" title="Số Căn cước công dân">
                            CCCD: {staff.citizenId || staff.cccd}
                          </span>
                        )}
                  ...
                  <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-[11px]">
                    ...
                    {hasExamCrafter && (
                      <span className="text-violet-900 font-medium">
                        Soạn đề: <strong className="font-mono font-bold">{formatVND(rates.gradingRate || staff.baseRate || 150000)}đ</strong>
                      </span>
                    )}
                  </div>
              ...
              {/* Card Footer Actions - Clean 3-column layout, no wrapping */}
              <div className="grid grid-cols-3 gap-1.5 pt-2.5 border-t border-slate-100">
                <button
                  onClick={() => handleOpenModal(staff)}
                  className="inline-flex items-center justify-center gap-1 py-1.5 px-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors cursor-pointer whitespace-nowrap"
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-1 py-1.5 px-2 text-xs font-bold text-white bg-slate-900 hover:bg-slate-800 rounded-lg shadow-2xs transition-colors cursor-pointer whitespace-nowrap"
                  title="Đánh giá chất lượng công việc (KPI)"
                >
            ...
            <form onSubmit={handleSave} className="space-y-4">
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-3">
                ...
                <div className="space-y-3">
                  ...
                  {/* 5. Soạn đề thi */}
                  {isExamCrafter && (
                    <div className="bg-white p-3 rounded-lg border border-violet-200 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full bg-violet-500"></span>
                          <span className="text-xs font-bold text-violet-950">Soạn Đề Thi - Đơn giá mỗi đề soạn:</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <input
                            type="number"
                            step="10000"
                            value={formData.gradingRate || 150000}
                            onChange={e => setFormData({ ...formData, gradingRate: Number(e.target.value) })}
                            className="w-28 px-2 py-1 bg-slate-50 border border-slate-200 rounded text-xs font-mono font-bold text-right"
                          />
                          <span className="text-[11px] text-slate-600">đ/đề</span>
                        </div>
                      </div>
                    </div>
                  )}
      ...
      {/* Employment Contract Export Modal */}
      {contractStaff && (
        <ContractModal
          staff={contractStaff}
